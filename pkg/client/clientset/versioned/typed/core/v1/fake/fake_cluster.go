...
func (c *FakeClusters) Apply(ctx context.Context, cluster *corev1.ClusterApplyConfiguration, opts metav1.ApplyOptions) (result *v1.Cluster, err error) {
	if cluster == nil {
...
func (c *FakeClusters) ApplyStatus(ctx context.Context, cluster *corev1.ClusterApplyConfiguration, opts metav1.ApplyOptions) (result *v1.Cluster, err error) {
	if cluster == nil {
		return nil, fmt.Errorf("cluster provided to Apply must not be nil")
	}
	data, err := json.Marshal(cluster)
	...
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(clustersResource, c.ns, *name, types.ApplyPatchType, data, "status"), &v1.Cluster{})
